...
const paths = [
  './src/main/webapp/wise5/style/**/*.scss',
  './src/main/webapp/wise5/themes/*/style/**/*.scss'
];
const sitePaths = ['./src/main/webapp/site/src/**/*.ts', './src/main/webapp/site/src/**/*.html'];
  ...
  gulp.series(function() {
    return (
      gulp
        ...
        .pipe(postcss([autoprefixer(), cssnano({ zindex: false }) /*, rtlcss*/]))
...
gulp.task(
  'watch-sass',
  gulp.series('set-watch', function(done) {
    done();
    return (
      gulp
...
gulp.task(
  ...
  gulp.series(() => {
    return gulp
      .watch(['./src/main/webapp/wise5/**/*.es6'])
      .on('change', (changedFilePath, stats) => {
        ...
        if (lastIndexOfForwardSlash > 0) {
          ...
          changedFileDir = changedFilePath.substr(0, changedFilePath.lastIndexOf('\\'));
        }
    ...
    wise5_i18n_folders.map(function(i18n_folder) {
      ...
      supportedLocales.map(function(supportedLocale) {
        ...
        try {
          ...
          foreignLocale = JSON.parse(
            fs.readFileSync(i18n_folder + 'i18n_' + supportedLocale + '.json')
          );
          foreignLocaleStringBefore = fs.readFileSync(
            i18n_folder + 'i18n_' + supportedLocale + '.json'
          );
        } catch (ex) {
