...
to get-pollution ;; computes the number of CO2 molecules required. 
  ...
  let f1 fval Eat_meat ;; sets f1 to 0, 1, 2, 3, 4, depending on the user input
  let f2 fval Litter
   set car-pollution  f1 ;; non-walking pollutes a lot
  set truck-pollution 4 * f1 + 0 * f2 + 0 ;; walking, heat/cool, veg, and showers
  set factory-pollution 4 * f1 + 0 * f2 + 0  ;; walking, Al, heat/cool, elect, veg, showers
end

to-report fval [fstring]
  ;; Converts the choices in fstring into pollution multiplers
  if fstring = "Always" [report 4]
   if fstring = "Sometimes" [report 2]
   if fstring = "Never" [report 0]
end
