...
to get-pollution ;; computes the number of CO2 molecules required. 
  ...
  ;; For example, if the user reports that she is sometimes a vegitarian, f6 is set to 2 and this contribues 3x of this to truck and pollution  
  let f1 fval walk-to-school ;; sets f1 to 0, 1, 2, 3, 4, depending on the user input
  let f2 fval recycle-aluminum 
  let f3 fval stop-littering ;; not used--no effect
  let f4 fval reduce-heating/cooling
  let f5 fval save-electricity
  let f6 fval Vegetarian
  let f7 fval Recycle-paper ;; not used--no effect
  let f8 fval short-showers
  set car-pollution 4 * f1 ;; non-walking pollutes a lot
  set truck-pollution f1 + 3 * f4 + 3 * f6 + f8 ;; walking, heat/cool, veg, and showers
  set factory-pollution f1 + 2 * f2 + 3 * f4 + 2 * f5 + 3 * f6 + f8 ;; walking, Al, heat/cool, elect, veg, showers
end

to-report fval [fstring]
  ;; Converts the choices in fstring into pollution multiplers
  if fstring = "Always" [report 0]
  if fstring = "Often" [report 1]
  if fstring = "Sometimes" [report 2]
  if fstring = "Rarely" [report 3]
...
to move-and-pollute ;; moves cars and trucks, emits pollutants from them and factories, removes some, too
  ask car [
    fd .3
    set xxx xcor ]
  add-CO2 car-pollution xxx earth-top + 1
  ask truck [
    fd .2
    set xxx xcor ]
  add-CO2 truck-pollution xxx earth-top + 1
  ask factory [ set xxx xcor ]
  add-CO2 factory-pollution xxx earth-top + 3
  remove-CO2 num-CO2 / 5 ;; remove one-fifth of the CO2. 
end
...
WHAT IS IT?
-----------
This is a model of energy flow in the earth, some of the sources of pollution, and their consequences. It shows the earth as rose colored. On the earth surface is a green strip. Above that is a blue atmosphere and black space at the top. Clouds and pollution molecules can be added to the atmosphere. The CO2 molecules represent greenhouse gasses that block infrared light that the earth emits. 
