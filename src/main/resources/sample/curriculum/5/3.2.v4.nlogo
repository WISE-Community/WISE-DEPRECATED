...
globals [ sky-top earth-top temperature num-CO2 num-clouds starter sun-brightness flag]

to setup
  ca
  set sun-brightness 1
  setup-world
  set temperature 15 ; Initial temperature
...
to update-albedo
  if (pycor = earth-top) [ set pcolor 50 + .09 * albedo ]
end       
...
to setup-world
  ...
  ask patches [                           ;; set colors of the world
      ...
      update-albedo ] 
   create-heat 40 [ set heading random 360 ;; start with some heat energy in the earth
     ...
     set breed heat    
     set shape "dot"  ]
     create-sunray 20  [ set heading 160 set color yellow    ; put in some sunrays
...
to run-heat    ;; advances the heat energy turtles
  set temperature .99 * temperature + .01 * (-1 + .25 * count heat) ;; the temperature is related to the number of heat turtles
  plot temperature
  ask heat [
    fd .5 * ( random 11 ) / 10
...
WHAT IS IT?
-----------
How does the albedo affect the temperatue of the earth? 
...
Changes Jan 2010 by Robert Tinker to focus on albedo.
