                ...
                <tr>
                  ...
                  <td>
                    <c:choose>
                      ...
                      <c:otherwise>
                        <fmt:formatDate value="${lastLoginTime}" type="both" dateStyle="short" timeStyle="short" />
                      </c:otherwise>
                    </c:choose>
                  </td>
                </tr>
      ...
      <div class="contentPanel contentRight">
        ...
        <div class="panelContent">
          <div id="tabSystem" class="panelTabs">
            <ul style='height:1.9em;'>   <!-- HT: I don't know why but if I don't set height, the ul's height is much larger than it should be. -->
                                ...
                                <c:choose>
                                  <c:when test="${fn:length(studentRunInfo.workgroup.members) == 1}">
                                    <li class="startProject">
                                      <a href="${contextPath}/student/startproject?runId=${studentRunInfo.run.id}"
                                          id='${studentRunInfo.run.id}' class="wisebutton"><spring:message code="student.index.runProject"/></a>
                                    </li>
                                  </c:when>
                                  <c:otherwise>
                                    <li class="startProject">
                                      <a href='${contextPath}/student/teamsignin.html?runId=${studentRunInfo.run.id}'
                                          id='${studentRunInfo.run.id}' class="wisebutton"><spring:message code="student.index.runProject"/></a>
                                    </li>
                                  </c:otherwise>
                                </c:choose>
                          ...
                          <c:choose>
                            <c:when test="${studentRunInfo.workgroup != null}" >
                              <c:forEach var="member" varStatus="membersStatus" items="${studentRunInfo.workgroup.members}">
                                ...
                                <c:if test="${membersStatus.last=='false'}">&</c:if>
                              </c:forEach>
                            </c:when>
                            <c:otherwise>
                    ...
                    <table class="runTable" >
                      <tr class="projectMainRow">
                        ...
                        <td rowspan="5" style="width:27%; padding:2px;">
                          <ul class="studentActionList">
                            <li class="startProject">
                              <a href="${contextPath}/student/startproject?runId=${studentRunInfo.run.id}" class="wisebutton">
