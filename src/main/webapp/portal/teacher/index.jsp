					...
					<div class="panelContent">
						...
						<table id="teacherMessageTable">
							<tr>
								...
								<ul class="reminders">
									<!-- show reminders for all runs that this teacher owns -->
									<c:forEach var="run" items="${allCurrentRuns}">
											...
											<c:if test='${(run.archiveReminderTime.time - current_date.time) < 0}'>
												<li><spring:message code="teacher.index.yourProjectRun" /> <span style="font-weight:bold;">${run.name} (${run.id})</span> <spring:message code="teacher.index.hasBeenOpenSince" />
													<fmt:formatDate value="${run.starttime}" type="date" dateStyle="medium" timeStyle="short" />.
													 <spring:message code="teacher.index.doYouWantToArchive" />
													[<a class="runArchiveLink" id="archiveRun_runId=${run.id}&runName=<c:out value="${fn:escapeXml(run.name)}" />" title="<spring:message code="teacher.management.projectruntabs.archive_title"/>
														${run.name} (<spring:message code="run_id"/> ${run.id})"><spring:message code="teacher.index.yes" /></a> /
													<a class="extendReminderLink" id='extendReminder_${run.id}'><spring:message code="teacher.index.remindMeLater" /></a>]
												</li>
											</c:if>
