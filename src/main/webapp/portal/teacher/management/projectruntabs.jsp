							 	...
							 	<c:forEach var="run" items="${ended_run_list}">
							  	...
							  	<tr id="runTitleRow_${run.id}" class="runRow">
									...
									<td>
					                    ...
					                    <ul class="actionList">
								        	<li><a href="/wise/previewproject.html?projectId=${run.project.id}&versionId=${run.versionId}" target="_blank"><img class="icon" alt="preview" src="/wise/themes/default/images/icons/teal/screen.png" /><span><spring:message code="preview_tip"/></span></a></li>
								        </ul>
					                    <ul class="actionList">
					                    	<li><a class="researchTools" title="<spring:message code="teacher.management.projectruntabs.researcherTools"/>: ${run.name} (<spring:message code="run_id_label"/> ${run.id})" id="runId=${run.id}&gradingType=export"><img class="icon" alt="export" src="/wise/themes/default/images/icons/teal/save.png" /><span><spring:message code="teacher.management.projectruntabs.researcherTools"/> <spring:message code="teacher.management.projectruntabs.exportStudentData"/></span></a></li>
					                    	<sec:accesscontrollist domainObject="${run}" hasPermission="16">					    	
								    	  		<li><a class="activateRun" id="activateRun_runId=${run.id}&runName=<c:out value="${fn:escapeXml(run.name)}" />" title="<spring:message code="teacher.management.projectruntabs.restore_title"/> ${run.name} (<spring:message code="run_id_label"/> ${run.id})"><img class="icon" alt="archive" src="/wise/themes/default/images/icons/teal/unlock.png" /><span><spring:message code="teacher.management.projectruntabs.restore"/></span></a></li>
								    		</sec:accesscontrollist>							
										</ul>
									</td>
									<td style="display:none;">${run.starttime}</td>
