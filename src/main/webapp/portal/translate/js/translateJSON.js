...
function buildTable5() {
  var translationTable =
    ...
    '<b>Please copy and save the following into a file for backup.</b><br/><br/>You should be able to view your translations in WISE now by refreshing your browser. You can close this window and keep working, or if you are done,<br/><button onclick=\'notifyComplete("' +
    projectType +
    '")\'>Click here to notify the WISE Staff.</button>' +
    ...
    "<input id='saveButton' type='button' onClick='save(\"" +
    projectType +
    "\")' value='Save'></input><span id='loadingGif' style='display:none'><img src='common/wait30.gif'></img></div>" +
    "<table border='1' id='translationTable'>";

  // build the header row
  translationTable +=
    "<tr><th class='cell_key'>key</th><th>" + View.prototype.i18n.defaultLocale + '</th>';
  ...
  if (currentLanguage != '') {
    for (key in View.prototype.i18n[View.prototype.i18n.defaultLocale]) {
      ...
      translationTable +=
        ...
        key +
        "</td>\n<td><textarea style='border:none;width:100%;height:100%' class='englishText' readonly='true'>" +
        value +
        '</textarea></td>\n';
      if (View.prototype.i18n[currentLanguage][key]) {
        translationTable +=
          "<td class='cell_currentLanguage'><textarea style='height:100%;width:100%' class='foreignText' id='" +
          key +
          "'>" +
          View.prototype.i18n[currentLanguage][key].replace(/[\n]/g, '\\n') +
          '</textarea></td>\n';
      } else {
        translationTable +=
          "<td class='cell_currentLanguage'><textarea style='height:100%;width:100%' class='foreignText' id='" +
...
function buildTable_JSON() {
  ...
  // build the header row
  translationTable +=
    "<tr><th class='cell_key'>key</th><th>description</th><th>" +
    View.prototype.i18n.defaultLocale +
    '</th>';
  translationTable += "<th class='cell_currentLanguage'>" + currentLanguage + '</th>';
  translationTable += '</tr>\n\n';
    ...
    for (key in View.prototype.i18n[View.prototype.i18n.defaultLocale]) {
      ...
      if (View.prototype.i18n[currentLanguage][key]) {
        translationTable +=
          ...
          key +
          "'>" +
          View.prototype.i18n[currentLanguage][key].value.replace(/[\n]/g, '\\n') +
          '</textarea></td>\n';
      } else {
        translationTable +=
          "<td class='cell_currentLanguage'><textarea style='height:100%;width:100%' class='foreignText' id='" +
  ...
  $('textarea').change(function() {
    ...
    View.prototype.i18n[currentLanguage][key].description =
      View.prototype.i18n[View.prototype.i18n.defaultLocale][key].description;
    isDirty = true; // mark document as changed
    updateMissingTranslationsCount();
  });
...
View.prototype.retrieveLocale_JSON = function(locale, projectType) {
  $.ajax({
    url: 'translate/' + projectType + '/' + locale,
    async: false,
    dataType: 'json',
    success: function(obj) {
      console.log('retrieved:' + locale);
      View.prototype.i18n[locale] = obj;
    },
    error: function() {
      console.log('error retrieving:' + locale);
    }
  });
};
