...
function updateMissingTranslationsCount() {
  ...
  if (numTotalTranslations == numCompletedTranslations) {
    ...
    cheerfulText =
      'Take it one step at a time! A journey of a thousand miles begins with a single step.';
  ...
  var progressSpan =
    "<span id='progress' style='width:150px; border: 2px solid gray; position: absolute; height:18px;padding:0px'>" +
    "<span id='bar' style='position:absolute;height:18px;background-color:#76FF3A;width:" +
    completionPercentage +
    "%'></span>" +
    "<span id='percent' style='position:absolute; left:40%; font-weight:bold;'>" +
    completionPercentage +
    '%</span></span>';
  $('#numMissingTranslations').html(
    '| ' +
      progressSpan +
...
function notifyComplete(projectType) {
  ...
  alert(
    'Thanks, WISE Staff has been notified. You should now be able to view your translations in WISE by refreshing your browser. If you have questions, please contact us!'
  );
}
...
View.prototype.i18n.supportedLocales = ['ar', 'en', 'zh_TW', 'zh_CN', 'nl', 'es', 'ja'];
