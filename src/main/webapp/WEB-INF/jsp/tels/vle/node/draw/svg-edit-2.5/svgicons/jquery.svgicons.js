	...
	$.svgIcons = function(file, opts) {
			...
			if(opts.svgz) {
				...
				$.ajax({
					...
					success: function(data) {
						svgdoc = data;
						//$(function() { // need to remove in wise4 for some reason
							getIcons('ajax');
						//});
					},
					error: function(err) {
		...
		function fixIDs(svg_el, svg_num, force) {
			...
			defs.find('[id]').each(function(i) {
				...
				$(this).attr('id', new_id);
				var new_id2 = urlBase + '#' + new_id; // fix for wise4/firefox url() reference problems
				svg_el.find('[fill="url(#' + id + ')"]').each(function() {
					//$(this).attr('fill', 'url(#' + new_id + ')');
					$(this).attr('fill', 'url(' + new_id2 + ')');
				}).end().find('[stroke="url(#' + id + ')"]').each(function() {
					//$(this).attr('stroke', 'url(#' + new_id + ')');
					$(this).attr('stroke', 'url(' + new_id2 + ')');
