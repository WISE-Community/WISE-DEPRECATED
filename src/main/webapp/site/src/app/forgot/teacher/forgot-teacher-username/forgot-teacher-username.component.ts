...
export class ForgotTeacherUsernameComponent implements OnInit {
  ...
  setEmailNotFoundMessage() {
    const message = this.i18n(
      `We did not find a WISE account associated with that email. Please make sure you have typed your email address correctly.`
    );
  ...
  setFailedToSendEmailMessage() {
    const message = this.i18n(
      `The server has encountered an error and was unable to send an email to you. Please try again. If the error continues to occur, please contact us.`
    );
    this.setMessage(message);
  }
