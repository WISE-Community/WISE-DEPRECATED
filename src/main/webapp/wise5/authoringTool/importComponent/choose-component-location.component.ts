...
class ChooseComponentLocationController {
  ...
  static $inject = ['$state', '$stateParams', 'ConfigService', 'ProjectAssetService',
      'ProjectService', 'TeacherDataService', 'UtilService'];
  ...
  importComponentAfter(insertAfterComponentId: string) {
    this.importComponents(this.nodeId, insertAfterComponentId)
        .then((newComponents) => {
      ...
      this.$state.go('root.at.project.node', { projectId: this.ConfigService.getProjectId(),
          nodeId: this.nodeId, newComponents: newComponents})
    });
  }
  ...
  saveImportedComponentsEvent(newComponents: any) {
    ...
    this.TeacherDataService.saveEvent('AuthoringTool', this.nodeId, null, null, 'Authoring',
        'componentImported', { componentsImported: importedComponents });
  }
