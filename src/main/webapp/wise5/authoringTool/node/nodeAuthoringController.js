...
function () {
  function NodeAuthoringController($anchorScroll, $filter, $injector, $location, $mdDialog, $rootScope, $scope, $state, $stateParams, $timeout, ConfigService, NodeService, ProjectAssetService, ProjectService, TeacherDataService, UtilService) {
    ...
    this.showComponentAuthoringViews = true; // mapping from component id to whether the component checkbox is checked

    this.componentsToChecked = {};
    this.TeacherDataService.setCurrentNodeByNodeId(this.nodeId); // the available constraint actions
    ...
    this.componentTypes = [{
      ...
      componentType: 'Draw',
      componentName: this.UtilService.getComponentTypeLabel('Draw')
    }, {
      componentType: 'Embedded',
      componentName: this.UtilService.getComponentTypeLabel('Embedded')
    }, {
      componentType: 'Graph',
      componentName: this.UtilService.getComponentTypeLabel('Graph')
    }, {
      componentType: 'HTML',
      componentName: this.UtilService.getComponentTypeLabel('HTML')
    }, {
      componentType: 'Label',
      componentName: this.UtilService.getComponentTypeLabel('Label')
    }, {
      componentType: 'Match',
      componentName: this.UtilService.getComponentTypeLabel('Match')
    }, {
      ...
      componentType: 'Table',
      componentName: this.UtilService.getComponentTypeLabel('Table')
    }]; // select the first component type by default

    this.selectedComponent = this.componentTypes[0].componentType;
    this.node = this.ProjectService.getNodeById(this.nodeId); // get the step number e.g. 1.3

    this.nodePosition = this.ProjectService.getNodePositionById(this.nodeId); // get the components in the node
    ...
    this.currentNodeCopy = this.UtilService.makeCopyOfJSONObject(this.node); // populate the branch authoring if any

    this.populateBranchAuthoring(); // create the summernote rubric element id

    this.summernoteRubricId = 'summernoteRubric_' + this.nodeId; // the tooltip text for the insert WISE asset button
    ...
    this.$scope.$on('componentShowSubmitButtonValueChanged', function (event, args) {
      ...
      if (showSubmitButton) {
        ...
        _this.node.showSubmitButton = false; // turn on the save buttons for all components in this step
        ...
        if (_this.ProjectService.doesAnyComponentInNodeShowSubmitButton(_this.node.id)) {
          ...
          _this.node.showSubmitButton = false; // turn off the save buttons for all the components
  ...
  _createClass(NodeAuthoringController, [{
    ...
    value: function populateBranchAuthoring() {
      if (this.node.transitionLogic != null) {
        ...
        this.createBranchBranches = []; // get the number of branches
        ...
        for (var t = 0; t < this.node.transitionLogic.transitions.length; t++) {
          ...
          if (transition != null) {
            ...
            var branch = {}; // set the branch number for display purposes
            ...
            branch.items = this.UtilService.makeCopyOfJSONObject(this.ProjectService.idToOrder); // an array that will hold all the checked items in the branch path

            branch.checkedItemsInBranchPath = []; // set the transition into the branch so we can access it easily later

            branch.transition = transition; // add the branch to the array of branches

            this.createBranchBranches.push(branch); // get the transition criteria
            ...
            try {
              for (var _iterator2 = nodeIdsInBranch[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                ...
                if (item != null) {
                  ...
                  item.checked = true; // add the item to the array of checked items in this branch path
    ...
    value: function close() {
      this.$scope.$broadcast('exitNode', {
        nodeToExit: this.node
      });
    ...
    value: function authoringViewTransitionToNodeIdChanged() {
      ...
      */
      this.ProjectService.calculateNodeNumbers(); // save changes
    ...
    value: function transitionCriteriaNodeIdChanged(transitionCriteria) {
      if (transitionCriteria != null && transitionCriteria.params != null) {
        ...
        var nodeId = transitionCriteria.params.nodeId; // clear the params
    ...
    value: function transitionCriteriaComponentIdChanged(transitionCriteria) {
      if (transitionCriteria != null && transitionCriteria.params != null) {
        ...
        var componentId = transitionCriteria.params.componentId; // clear the params
    ...
    value: function deleteTransition(transition) {
      ...
      var answer = confirm(this.$translate('areYouSureYouWantToDeleteThisPath', {
        stepTitle: stepTitle
      }));
    ...
    value: function addComponentButtonClicked() {
      // select the first component type by default
      this.selectedComponent = this.componentTypes[0].componentType; // show the add component UI elements

      this.nodeAuthoringViewButtonClicked('addComponent'); // turn on add component mode

      this.turnOnAddComponentMode(); // turn on the move component mode

      this.turnOffMoveComponentMode(); // turn on the insert component mode

      this.turnOnInsertComponentMode(); // hide the component authoring
    ...
    value: function deleteComponent(componentId) {
      if (confirm(this.$translate('confirmDeleteComponent'))) {
        ...
        this.ProjectService.deleteComponent(this.nodeId, componentId); // check if we need to show the node save or node submit buttons
      ...
      if (components != null) {
        ...
        try {
          for (var _iterator4 = components[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            ...
            if (component != null) {
              var componentType = component.type; // get the service for the component type
    ...
    value: function authoringViewNodeChanged(parseProject) {
      ...
      this.undoStack.push(this.currentNodeCopy); // update the current node copy
    ...
    value: function undo() {
      if (this.undoStack.length === 0) {
        ...
        if (confirm(this.$translate('confirmUndoLastChange'))) {
          ...
          var nodeCopy = this.undoStack.pop(); // revert the node back to the previous version
    ...
    value: function getNewNodeConstraintId(nodeId) {
      ...
      var constraintCounter = 1; // loop until we have found an unused constraint id

      while (newNodeConstraintId == null) {
        // create a potential constraint id
        var potentialNewNodeConstraintId = nodeId + 'Constraint' + constraintCounter; // check if the constraint id has been used already
    ...
    value: function addConstraint() {
      ...
      var newNodeConstraintId = this.getNewNodeConstraintId(this.nodeId); // create the constraint object
      ...
      constraint.removalCriteria.push(removalCriteria); // create the constraints array if it does not exist
    ...
    value: function removalCriteriaNameChanged(criteria) {
      if (criteria != null) {
        // clear the params
        criteria.params = {}; // get the params for the given criteria name
    ...
    value: function nodeAuthoringViewButtonClicked(view) {
      if (view == 'addComponent') {
        ...
        this.showCreateBranch = false; //this.showAdvanced = false;
        ...
        this.showEditButtons = false;
        this.showRubric = false;
        this.showCreateBranch = !this.showCreateBranch; //this.showAdvanced = false;
    ...
    value: function createBranchNumberOfBranchesChanged() {
      if (this.createBranchNumberOfBranches == 0) {
        ...
        alert(this.$translate('errorYouCantHave0BranchPaths')); // revert the number of branch paths value
         ...
         */
        var answer = confirm(this.$translate('areYouSureYouWantToReduceTheNumberOfBranchesToX', {
        ...
        for (var b = 0; b < this.createBranchNumberOfBranches; b++) {
          if (b >= this.createBranchBranches.length) {
            ...
            var _branch = {}; // set the branch number
            ...
            _branch.items = this.UtilService.makeCopyOfJSONObject(this.ProjectService.idToOrder); // add the branch to the array of branches

            this.createBranchBranches.push(_branch); // create a transition to represent the branch
            ...
            } else if (this.createBranchCriterion == 'workgroupId') {// workgroup id branching does not require a transition criterion
            } else if (this.createBranchCriterion == 'random') {} // random branching does not require a transition criterion
            // add the transition


            this.node.transitionLogic.transitions.push(transition); // save a reference to the transition in the branch
    ...
    value: function createBranchNodeIdChanged() {
      ...
      this.createBranchUpdateTransitions(); // save the project
        ...
        if (branch != null) {
          ...
          if (transition != null) {
            if (this.createBranchCriterion == 'choiceChosen') {
              ...
              transition.criteria = []; // create a new choice chosen criterion
              ...
              // clear the criteria array
              transition.criteria = []; // create a new score criterion
              ...
              branch.choiceId = null; // set the scores into the branch object
               ...
               */
              delete transition['criteria']; // clear the node id and component id
    ...
    value: function createBranchUpdateChoiceChosenIdsHelper(component) {
      var nodeId = this.createBranchNodeId;
      var componentId = this.createBranchComponentId; // get the choices from the component
      ...
      if (choices != null) {
        ...
        for (var c = 0; c < choices.length; c++) {
          ...
          if (choice != null) {
            ...
            var isCorrect = choice.isCorrect; // get the branch that corresponds to the choice

            var branch = this.createBranchBranches[c];

            if (branch != null) {
              // get the choice for this branch
              branch.choiceId = id; // make a copy of the choices from the component

              branch.choices = this.UtilService.makeCopyOfJSONObject(choices); // get the transition corresponding to the branch
    ...
    value: function createBranchStepClicked(branch, item) {
      ...
      var orderedItems = this.$filter('orderBy')(this.$filter('toArray')(branch.items), 'order'); // an array that will hold the items that were checked

      branch.checkedItemsInBranchPath = [];
      var checkedItemsInBranchPath = branch.checkedItemsInBranchPath; // an array that will hold the node ids that were checked

      branch.nodeIdsInBranch = []; // used to hold the previously checked node id

      var previousCheckedNodeId = null; // the node id after the node that was clicked
      ...
      for (var i = 0; i < orderedItems.length; i++) {
        ...
        if (orderedItem != null && orderedItem.checked) {
            ...
            if (previousCheckedNode != null) {
              ...
              if (transitionLogic != null) {
                if (transitionLogic.transitions != null) {
                  ...
                  transitionLogic.transitions = []; // create a new transition object to the current item
          ...
          checkedItemsInBranchPath.push(orderedItem); // add the node id to the array of node ids in the branch path

          branch.nodeIdsInBranch.push(orderedItem.$key); // remember the previously checked node id
        ...
        if (_node != null) {
          ...
          if (_transitionLogic != null) {
            if (_transitionLogic.transitions != null) {
              // clear the transitions
              _transitionLogic.transitions = []; // make a transition to the merge point

              var _transition3 = {};
              _transition3.to = this.createBranchMergePointNodeId; // add the transition
      ...
      var branchNumber = branch.number; // get the node id that was clicked

      var nodeId = item.$key; // get the transition that corresponds to the branch
      ...
      if (node != null) {
        ...
        if (item.checked) {
          ...
          var toNodeId = firstNodeId; // add the branch path taken constraints
      ...
      try {
        for (var _iterator9 = checkedItemsInBranchPath[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
          ...
          var _toNodeId = firstNodeId; // add the branch path taken constraints
    ...
    value: function createBranchMergePointNodeIdChanged() {
              ...
              if (lastNodeIdInBranchPath != null) {
                ...
                if (lastNodeInBranchPath != null) {
                  ...
                  if (transitionLogic != null) {
                    if (transitionLogic.transitions != null) {
                      // clear the transitions
                      transitionLogic.transitions = []; // make a new transition to the merge point
      ...
      this.ProjectService.calculateNodeNumbers(); // save the project
    ...
    value: function removeBranch() {
      ...
      var nodeId = this.node.id; // get the node id that comes after this node
      ...
      this.ProjectService.setTransition(nodeId, nodeIdAfter); // clear the transition logic fields
      ...
      this.ProjectService.setTransitionLogicField(nodeId, 'maxPathsVisitable', null); // clear the branch authoring fields
      ...
      branch.checkedItemsInBranchPath = [];
      var transition = null; // get the transition from the node
    ...
    value: function removeBranchPath(branch) {
      if (branch != null) {
        ...
        var branchPathIndex = this.createBranchBranches.indexOf(branch); // remove the branch path

        this.createBranchBranches.splice(branchPathIndex, 1); // remove the transition that corresponds to the branch path
    ...
    value: function summernoteRubricHTMLChanged() {
      ...
      html = this.UtilService.insertWISELinks(html); // update the step rubric

      this.node.rubric = html; // save the project
        ...
        for (var c = 0; c < this.components.length; c++) {
          ...
          if (component != null && component.id != null) {
            ...
            if (checked) {
              ...
              var componentNumberAndType = c + 1 + '. ' + component.type; // the component is checked
    ...
    value: function importButtonClicked() {
      ...
      this.importProject = null; // hide the other views
    ...
    value: function moveButtonClicked() {
      if (this.getSelectedComponentIds().length == 0) {
        ...
        this.nodeAuthoringViewButtonClicked('move'); // turn off add component mode

        this.turnOffAddComponentMode(); // turn on the move component mode

        this.turnOnMoveComponentMode(); // turn on the insert component mode
    ...
    value: function copyButtonClicked() {
      if (this.getSelectedComponentIds().length == 0) {
        ...
        // hide the other views
        this.nodeAuthoringViewButtonClicked('copy'); // turn on the move component mode

        this.turnOnCopyComponentMode(); // turn on the insert component mode

        this.turnOnInsertComponentMode(); // hide the component authoring views
    ...
    value: function deleteButtonClicked() {
      ...
      if (this.getSelectedComponentIds().length == 0) {
        ...
        this.$timeout(function () {
          var confirmMessage = ''; // get the selected component numbers and types
          ...
          for (var c = 0; c < selectedComponentNumbersAndTypes.length; c++) {
            ...
            var selectedComponentNumberAndType = selectedComponentNumbersAndTypes[c]; // show the component number and type in the message
          ...
          if (confirm(confirmMessage)) {
            var selectedComponents = _this4.getSelectedComponentIds(); // data saved in the component deleted event
            ...
            _this4.saveEvent('componentDeleted', 'Authoring', data); // check if we need to show the node save or node submit buttons
          ...
          _this4.$timeout(function () {
            ...
            _this4.turnOffInsertComponentMode(); // uncheck the component check boxes


            _this4.clearComponentsToChecked(); // show the component authoring
    ...
    value: function checkIfNeedToShowNodeSaveOrNodeSubmitButtons() {
      if (this.ProjectService.doesAnyComponentInNodeShowSubmitButton(this.nodeId)) {
        ...
        if (this.ProjectService.doesAnyComponentHaveWork(this.nodeId)) {
          ...
          this.node.showSubmitButton = false; // hide the save button in all the components
    ...
    value: function handleAddComponent(componentId) {
      var newComponents = []; // create a component and add it to this node
    ...
    value: function handleCopyComponent(componentId) {
      var newComponents = [];
      var selectedComponentIds = this.getSelectedComponentIds(); // data saved in the component copied event

      var data = {};
      var componentsCopied = this.getComponentObjectsForEventData(selectedComponentIds); // copy the components to their new location

      newComponents = this.ProjectService.copyComponentAndInsert(this.nodeId, selectedComponentIds, componentId); // get the information for all the components that were copied
    ...
    value: function showImportProject(importProjectId) {
      ...
      if (this.importProjectId == null) {
        ...
        this.ProjectService.retrieveProjectById(this.importProjectId).then(function (projectJSON) {
          ...
          _this7.importProject = projectJSON; // calculate the node order of the import project
    ...
    value: function backButtonClicked() {
      if (this.showImportView || this.showRubric || this.showAdvanced) {
        this.UtilService.hideJSONValidMessage(); // we are in the import view so we will go back to the node view
    ...
    value: function autoSaveJSON() {
      ...
      try {
        ...
        var updatedNode = angular.fromJson(this.authoringNodeContentJSONString); // set the updated node into the project

        this.ProjectService.setNode(this.nodeId, updatedNode); // set the updated node into this controller

        this.node = updatedNode; // set the components into this controller

        this.components = this.ProjectService.getComponentsByNodeId(this.nodeId); // set the current node

        this.TeacherDataService.setCurrentNodeByNodeId(this.nodeId); // update the branch authoring fields into the controller

        this.populateBranchAuthoring(); // save the project
    ...
    value: function componentAdvancedButtonClicked(componentId) {
      this.$rootScope.$broadcast('componentAdvancedButtonClicked', {
...
var _default = NodeAuthoringController;
