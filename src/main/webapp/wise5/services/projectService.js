    ...
    _createClass(ProjectService, [{
        ...
        value: function getCurrentAuthors() {
            ...
            return this.$q(function (resolve, reject) {
                ...
                var notifyProjectEndURL = _this4.ConfigService.getConfigParam('getCurrentAuthorsURL') + projectId;
                var httpParams = {};
                httpParams.method = 'GET';
                httpParams.url = notifyProjectEndURL;

                _this4.$http(httpParams).then(function (result) {
                    resolve(result.data);
                });
            });
        }
            ...
            return this.$q(function (resolve, reject) {
                if (projectId == null) {
                    if (_this5.project != null) {
                        projectId = _this5.ConfigService.getProjectId();
                    } else {
                        resolve();
                    }
                }
                var notifyProjectEndURL = _this5.ConfigService.getConfigParam('notifyProjectEndURL') + projectId;
                var httpParams = {};
                httpParams.method = 'POST';
                httpParams.url = notifyProjectEndURL;

                _this5.$http(httpParams).then(function () {
                    resolve();
                });
            });
        }
        ...
        value: function copyNodes(selectedNodes, fromProjectId, toProjectId) {
            ...
            return this.$http(httpParams).then(function (result) {
                ...
                var inactiveNodes = _this6.getInactiveNodes();
                ...
                for (var n = 0; n < selectedNodes.length; n++) {
                    ...
                    if (selectedNode != null) {
                        ...
                        var tempNode = _this6.UtilService.makeCopyOfJSONObject(selectedNode);

                        // check if the node id is already being used in the current project
                        if (_this6.isNodeIdUsed(tempNode.id)) {
                            // the node id is already being used in the current project

                            // get the next available node id
                            var nextAvailableNodeId = _this6.getNextAvailableNodeId();
