...
nodesByToNodeId.push(node);}}}}}}}return nodesByToNodeId;}},{key:'getTransitionsByFromAndToNodeId',value:function getTransitionsByFromAndToNodeId(fromNodeId,toNodeId){var transitionsResults=[];if(fromNodeId!=null&&toNodeId!=null){var node=this.getNodeById(fromNodeId);if(node!=null){var transitionLogic=node.transitionLogic;if(transitionLogic!=null){var transitions=transitionLogic.transitions;if(transitions!=null){for(var t=0;t<transitions.length;t++){var transition=transitions[t];if(transition!=null){var to=transition.to;if(toNodeId===to){transitionsResults.push(transition);}}}}}}}return transitionsResults;}},{key:'getLayoutLogic',value:function getLayoutLogic(){var layoutLogic=null;var project=this.getProject();if(project!=null){layoutLogic=project.layoutLogic;}return layoutLogic;}},{key:'retrieveProject', /**
     ...
     */value:function retrieveProject(){var _this=this;var projectURL=this.ConfigService.getConfigParam('projectURL');if(projectURL!=null){return this.$http.get(projectURL).then(function(result){var projectJSON=result.data;_this.setProject(projectJSON);return projectJSON;});}else {return null;}}},{key:'saveProject',value:function saveProject(projectJSON,commitMessage){if(projectJSON==null){ // get the project from this service
projectJSON=angular.toJson(this.project,4);}var httpParams={};httpParams.method='POST';httpParams.url=this.ConfigService.getConfigParam('saveProjectURL');httpParams.headers={'Content-Type':'application/x-www-form-urlencoded'};var params={};params.projectId=this.ConfigService.getProjectId();params.commitMessage=commitMessage;params.projectJSONString=projectJSON;httpParams.data=$.param(params);return this.$http(httpParams).then(angular.bind(this,function(result){var commitHistory=result.data;return commitHistory;}));}},{key:'registerNewProject',value:function registerNewProject(projectJSON,commitMessage){if(!commitMessage){commitMessage="";}var httpParams={};httpParams.method='POST';httpParams.url=this.ConfigService.getConfigParam('registerNewProjectURL');httpParams.headers={'Content-Type':'application/x-www-form-urlencoded'};var params={};params.commitMessage=commitMessage;params.projectJSONString=projectJSON;httpParams.data=$.param(params);return this.$http(httpParams).then(function(result){var projectId=result.data;return projectId;});}},{key:'commitChanges',value:function commitChanges(commitMessage){var commitProjectURL=this.ConfigService.getConfigParam('commitProjectURL');return this.$http({url:commitProjectURL,method:'POST',params:{commitMessage:commitMessage}}).then(angular.bind(this,function(result){var commitResult=result.data;return commitResult;}));}},{key:'getCommitHistory',value:function getCommitHistory(projectId){var commitProjectURL=this.ConfigService.getConfigParam('commitProjectURL');return this.$http({url:commitProjectURL,method:'GET'}).then(angular.bind(this,function(result){return result.data;}));}},{key:'getThemePath',value:function getThemePath(){var wiseBaseURL=this.ConfigService.getConfigParam('wiseBaseURL');var project=this.getProject();if(project&&project.theme){ // TODO: check if this is a valid theme (using ConfigService) rather than just truthy
