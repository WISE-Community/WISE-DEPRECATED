    ...
    var ensureIds = function (series) {
      ...
      angular.forEach(series, function (s) {
        if (!angular.isDefined(s.id)) {
          ...
          // increment the seriesId counter
          seriesId++;

          // get the next unused series id
          s.id = getNextUnusedSeriesId(usedSeriesIds);

          changed = true;
        }

        // add the series id to our array of used series ids
        usedSeriesIds.push(s.id);
      });
      return changed;
    };

    /**
     * Get the next unused series id based upon the existing
     * series ids
     * @param usedSeriesIds an array of used series ids
     * @returns a string containing the next unused series id
     */
    var getNextUnusedSeriesId = function(usedSeriesIds) {
      var nextUnusedSeriesId = null;

      // check if any of the next 100 series ids are used
      for (var x = 0; x < 100; x++) {
        // get the next potential series id
        var tempSeriesId = 'series-' + (seriesId + x);

        // check if the next potential series id is used
        if (usedSeriesIds.indexOf(tempSeriesId) == -1) {
          // the next potential series id is not used so we will use it
          nextUnusedSeriesId = tempSeriesId;
          seriesId = seriesId + x;
          break;
        }
      }

      return nextUnusedSeriesId;
    };
    // immutable
    var axisNames = [
        'xAxis',
