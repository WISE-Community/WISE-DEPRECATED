...
describe('WISE Authoring Tool Info View', () => {
  ...
  it('should allow user to change the project title', () => {
    ...
    element
      .all(by.repeater('metadataField in projectInfoController.metadataAuthoring.fields'))
      .then(metadataField => {
        const titleInput = metadataField[0].element(
          by.model('projectInfoController.metadata[metadataField.key]')
        );
