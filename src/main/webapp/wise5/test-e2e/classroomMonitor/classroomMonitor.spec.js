      ...
      browser
        ...
        .then(() => {
          ...
          expect(browser.getCurrentUrl()).toMatch(
            'http://localhost:8080/wise/classroomMonitor/[0-9]+#!/project/'
          );
          expect(browser.getTitle()).toEqual('WISE Classroom Monitor');
          expect(
            element(by.cssContainingText('top-bar', 'My Awesome Science Project')).isDisplayed()
          ).toBeTruthy(); // check that the title of the run is displayed
          expect(
            element(by.cssContainingText('md-toolbar', 'Grade By Step')).isDisplayed()
          ).toBeTruthy(); // check Grade by Step view is displayed
          ...
          let pauseScreensButton = element(
            by.xpath('//button[@aria-label="Lock Student Screens"]')
          );
          let pauseScreensMenu = element(
            by.cssContainingText('.md-open-menu-container', 'Lock Student Screens')
          );
          ...
          element
            .all(by.repeater('id in nodeProgressController.rootNode.ids'))
            .then(groupNavItems => {
