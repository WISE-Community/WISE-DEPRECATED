...
describe('WISE5 Notebook in Preview Mode', () => {
  ...
  it('should snip image to notebook', () => {
    ...
    let addNoteCancelButton = addNoteDialog.element(
      by.xpath('//md-dialog-actions/button[@aria-label="Cancel"]')
    );
    ...
    nyanCatImage.click(); // click on the nyan cat image
    expect(addNoteDialog.isDisplayed()).toBeTruthy(); // add note dialog should appear
    let addNoteSaveButton = addNoteDialog.element(
      by.xpath('//md-dialog-actions/button[@aria-label="Save"]')
    );
  ...
  it('should open and close the notebook notes view', () => {
    ...
    let closeNotebookButton = element(
      by.xpath('//notebook-notes/md-sidenav/md-toolbar/div/button[@aria-label="Close"]')
    );
