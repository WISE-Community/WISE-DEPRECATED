...
describe('WISE5 Student VLE Preview', () => {
  ...
  it('should show first step and default UI elements on the page', () => {
    ...
    common.shouldBePresent(
      ...
      vle.accountButton,
      vle.accountMenu,
      vle.notificationButton,
      vle.notificationMenu
    );
    common.shouldBeHidden(vle.accountMenu, vle.notificationMenu);
    const nodeContent = element(
      by.cssContainingText(
        '.node-content',
        'This is a step where authors can enter their own html.'
      )
    );
  ...
  it('should navigate next and previous steps using the buttons', () => {
    ...
    let nodeContent = element(
      by.cssContainingText('.node-content', 'This is a step where students enter text.')
    );
    common.shouldBePresent(nodeContent);
    ...
    nodeContent = element(
      by.cssContainingText('.node-content', 'Explain how the sun helps animals survive.')
    );
    common.shouldBePresent(nodeContent);
  ...
  it('should allow user to jump to a step using the navigation drop-down menu', () => {
    ...
    element
      .all(by.repeater("item in stepToolsCtrl.idToOrder | toArray | orderBy : 'order'"))
      .then(stepSelectOptions => {
        expect(stepSelectOptions[1].element(by.css('.node-select__text')).getText()).toBe(
          '1.1: HTML Step'
        );
        expect(stepSelectOptions[7].element(by.css('.node-select__text')).getText()).toBe(
  ...
  it('should display the group view and allow user to collapse/expand group navitems', () => {
    ...
    element.all(by.repeater('id in navCtrl.rootNode.ids')).then(groupNavItems => {
      ...
      activity1.all(by.repeater('childId in navitemCtrl.item.ids')).then(stepNavItems => {
        ...
        expect(
          stepNavItems[0]
            .element(by.cssContainingText('.material-icons', 'check_circle'))
            .isPresent()
        ).toBeTruthy();
  ...
  it('should allow preview user to view the notification menu', () => {
    ...
    const notificationDialogTitle = element(
      by.xpath('//md-toolbar/span/span[@translate="notificationsTitle"]')
    );
    expect(notificationDialogTitle.isDisplayed()).toBeTruthy();
    expect(notificationDialogTitle.getText()).toEqual('Alerts');

    const notificationDialogContent = element(
      by.xpath('//md-content/div/span[@translate="noAlerts"]')
    );
