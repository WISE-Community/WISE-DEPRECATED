...
class NavItemController {
  ...
  $onInit() {
    ...
    if (this.isParentGroupPlanning) {
       ...
       * nodes that can be used in this group
       */
      this.availablePlanningNodes = this.PlanningService.getAvailablePlanningNodes(
        this.parentGroupId
      );
        ...
        if (oldNode) {
          ...
          if (this.StudentDataService.previousStep) {
            this.$scope.$parent.isPrevStep =
              this.nodeId === this.StudentDataService.previousStep.id;
          }
        }
    ...
    let scroll = this.autoScroll([document.querySelector('#content')], {
      margin: 30,
      pixels: 50,
  ...
  addPlanningNodeInstanceInside(nodeIdToInsertInside, templateNodeId) {
    ...
    let planningNodeInstance = this.PlanningService.createPlanningNodeInstance(
      templateNodeId,
      nextAvailablePlanningNodeId
    );
  ...
  addPlanningInstance(event, targetNodeId, insertInside) {
    ...
    let choosePlanningItemTemplateUrl =
      this.ProjectService.getThemePath() + '/themeComponents/navItem/choosePlanningItem.html';
    ...
    function ChoosePlanningItemController(
      $scope,
      $mdDialog,
      targetNodeId,
      insertInside,
    ...
    ChoosePlanningItemController.$inject = [
      '$scope',
      '$mdDialog',
      'targetNodeId',
      'insertInside',
  ...
  togglePlanningMode() {
    ...
    this.$rootScope.$broadcast('togglePlanningMode', {
      nodeId: this.nodeId,
