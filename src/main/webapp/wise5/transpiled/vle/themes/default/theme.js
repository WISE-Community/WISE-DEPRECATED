...
define(['angular', /*'annotationService',*/'configService', 'nodeService', 'notebookService', 'projectService', 'sessionService', 'studentDataService'], function (angular, /*AnnotationService,*/ConfigService, NodeService, NotebookService, ProjectService, SessionService, StudentDataService) {

    angular.module('app.theme', []).directive('navItem', function () {
        return {
            scope: {
                nodeId: '=',
                showPosition: '=',
    ...
    }).controller('NavItemController', function ($scope, $state, $stateParams, $element, ProjectService, StudentDataService) {
        ...
        $scope.$watch(function () {
            return StudentDataService.currentNode;
        }, function (newNode) {
        ...
        var zoomToElement = function zoomToElement() {
            setTimeout(function () {
    ...
    }).controller('StepToolsCtrl', function ($scope, $state, $stateParams, $element, NodeService, ProjectService, StudentDataService) {
    ...
    }).controller('NodeStatusIconCtrl', function ($scope, $state, $stateParams, $element, ProjectService, StudentDataService) {
    ...
    .controller('ThemeController', function ($scope, $state, $stateParams, $document, ConfigService, ProjectService, StudentDataService, NodeService, NotebookService, SessionService, $mdDialog, $mdSidenav, $mdToast, $mdComponentRegistry) {
        ...
        $scope.$on('nodeClickLocked', angular.bind(this, function (event, args) {
            ...
            $mdDialog.show($mdDialog.alert().parent(angular.element(document.body)).title('Item Locked').content('Sorry, you cannot view this item yet.').ariaLabel('Item Locked').clickOutsideToClose(true).ok('OK').targetEvent(event));
        }));
        ...
        $scope.$on('notebookAddDuplicateAttempt', angular.bind(this, function (event, args) {
            $mdDialog.show($mdDialog.alert().parent(angular.element(document.body)).title('Item already exists in Notebook').content('You can add another version of the item by making changes and then adding it again.').ariaLabel('Notebook Duplicate').clickOutsideToClose(true).ok('OK').targetEvent(event));
        }));
    ...
    }).controller('ProjectStatusController', function ($scope, $state, $stateParams, projectStatus, userNames) {
