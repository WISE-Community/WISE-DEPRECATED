...
(function (window) {
  ...
  var Recorder = function Recorder(source, audioRecorderController, cfg) {
    ...
    encoderWorker.audioRecorderController = audioRecorderController;
    var worker = new Worker(config.workerPath || WORKER_PATH);
    worker.audioRecorderController = audioRecorderController;
    worker.postMessage({
      command: 'init',
      config: {
      ...
      fileReader.onload = function () {
        ...
        encoderWorker.onmessage = function (e) {
          if (e.data.cmd == 'data') {
            ...
            var mp3file = new File([mp3Blob], mp3Name, {
              lastModified: now, // optional - default = now
              type: 'audio/mp3' // optional - default = ''
            });
            this.audioRecorderController.uploadAudioAsset(mp3file);
