...
define(function () { 'use strict';
    ...
    init: function init(initOpt) {
      ...
      if (utils.documentIsReady()) {
        ...
        if (document.addEventListener) {
          _appendToContainer2 = function appendToContainer() {
            document.removeEventListener('DOMContentLoaded', _appendToContainer2);
            window.removeEventListener('load', _appendToContainer2);
            utils.getContainer(opt).appendChild(el);
          };

          document.addEventListener('DOMContentLoaded', _appendToContainer2, false);
        }
        // IE
        else {
            _appendToContainer2 = function _appendToContainer() {
              if (document.readyState === 'complete') {
                document.detachEvent('onreadystatechange', _appendToContainer2);
                window.detachEvent('onload', _appendToContainer2);
                utils.getContainer(opt).appendChild(el);
              }
            };

            document.attachEvent('onreadystatechange', _appendToContainer2);
          }
        utils.addEvtListener(window, 'load', _appendToContainer2);
      }
    }
  };
  ...
  Hopscotch = function Hopscotch(initOptions) {
    ...
    adjustWindowScroll = function adjustWindowScroll(cb) {
      ...
      // Get the container
      containerEl = utils.getContainer({ container: getOption('container') }),


      // Calculate the target element top and bottom position
      targetEl = utils.getStepTarget(getCurrStep()),
          targetBounds = targetEl.getBoundingClientRect(),
          targetElTop = targetBounds.top + containerEl.scrollTop,
          targetElBottom = targetBounds.bottom + containerEl.scrollHeight,
      ...
      windowTop = containerEl.scrollTop,
          windowBottom = windowTop + containerEl.scrollHeight,
      ...
      // Abrupt scroll to scroll target
          ...
          containerEl.scrollTo(0, scrollToVal);
        ...
        // Smooth scroll to scroll target
            ...
            // Use jQuery if it exists
                ...
                $(containerEl).animate({ scrollTop: scrollToVal }, getOption('scrollDuration'), cb);
              }
