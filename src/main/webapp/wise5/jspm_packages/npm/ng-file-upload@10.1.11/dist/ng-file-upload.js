...
(function(process) {
  ...
  ngFileUpload.service('Upload', ['$parse', '$timeout', '$compile', '$q', 'UploadExif', function($parse, $timeout, $compile, $q, UploadExif) {
    ...
    function resize(files, attr, scope) {
      ...
      if (!param || !upload.isResizeSupported() || !files.length)
      ...
      angular.forEach(files, function(f, i) {
        if (f.type.indexOf('image') === 0) {
          if (param.pattern && !upload.validatePattern(f, param.pattern))
            return;
          var promise = upload.resize(f, param.width, param.height, param.quality, param.type, param.ratio, param.centerCrop);
    ...
    upload.updateModel = function(ngModel, attr, scope, fileChange, files, evt, noDelay) {
      ...
      if (keepResult.keep && (!newFiles || !newFiles.length))
        return;
