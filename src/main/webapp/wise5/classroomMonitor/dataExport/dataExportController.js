...
function () {
  function DataExportController($filter, $injector, $mdDialog, $rootScope, $scope, $state, AnnotationService, ConfigService, FileSaver, MatchService, ProjectService, StudentStatusService, TeacherDataService, TeacherWebSocketService, UtilService) {
    ...
    this.project = this.ProjectService.project; // create the mapping of node id to order
    ...
    this.projectItems = nodeOrderOfProject.nodes; // save event when data export view is displayed
  ...
  _createClass(DataExportController, [{
    ...
    value: function exportStudentWork(exportType) {
      ...
      this.TeacherDataService.getExport("allStudentWork", selectedNodes).then(function (result) {
        ...
        var workgroups = _this.ConfigService.getClassmateUserInfosSortedByWorkgroupId(); // get the run id


        var runId = _this.ConfigService.getRunId(); // the rows that will show up in the export


        var rows = []; // the counter for the rows

        var rowCounter = 1; // mapping from column name to column number

        var columnNameToNumber = {}; // an array of column names
        ...
        var headerRow = []; // generate the header row by looping through all the column names
        ...
        if (workgroups != null) {
          ...
          for (var w = 0; w < workgroups.length; w++) {
            ...
            if (workgroup != null) {
              ...
              var componentRevisionCounter = {}; // get the component states for the workgroup
        ...
        var fileName = ""; // make the file name
    ...
    value: function createStudentWorkExportRow(columnNames, columnNameToNumber, rowCounter, workgroupId, wiseId1, wiseId2, wiseId3, studentName1, studentName2, studentName3, periodName, componentRevisionCounter, componentState) {
      ...
      row.fill(""); // set the row number

      row[columnNameToNumber["#"]] = rowCounter; // set workgroup id
      ...
      row[columnNameToNumber["Class Period"]] = periodName; // set the project id

      row[columnNameToNumber["Project ID"]] = this.ConfigService.getProjectId(); // set the project name

      row[columnNameToNumber["Project Name"]] = this.ProjectService.getProjectTitle(); // set the run id

      row[columnNameToNumber["Run ID"]] = this.ConfigService.getRunId(); // set the student work id
      ...
      if (componentState.serverSaveTime != null) {
        ...
        var formattedDateTime = this.UtilService.convertMillisecondsToFormattedDateTime(componentState.serverSaveTime); // set the time stamp string e.g. Wed Apr 06 2016 9:05:38 AM
      ...
      row[columnNameToNumber["Node ID"]] = componentState.nodeId; // set the component id

      row[columnNameToNumber["Component ID"]] = componentState.componentId; // set the step title

      row[columnNameToNumber["Step Title"]] = this.ProjectService.getNodePositionAndTitleByNodeId(componentState.nodeId); // get the component part number

      var componentPartNumber = this.ProjectService.getComponentPositionByNodeIdAndComponentId(componentState.nodeId, componentState.componentId) + 1; // set the component part number

      row[columnNameToNumber["Component Part Number"]] = componentPartNumber; // get the component
      ...
      if (component != null) {
        ...
        if (component.prompt != null) {
          ...
          var prompt = this.UtilService.removeHTMLTags(component.prompt); // replace " with ""

          prompt = prompt.replace(/"/g, '""'); // set the prompt with the html tags removed
    ...
    value: function incrementRevisionCounter(componentRevisionCounter, nodeId, componentId) {
      ...
      var revisionCounter = componentRevisionCounter[nodeIdAndComponentId]; // increment the revision counter
          ...
          if (row != null) {
            ...
            for (var c = 0; c < row.length; c++) {
              ...
              if (cell == null || cell === '' || typeof cell === 'undefined') {
                 ...
                 */
                // convert the
                cell = JSON.stringify(cell); // replace " with ""
    ...
    value: function exportEvents() {
      ...
      this.TeacherDataService.getExport("events", selectedNodes).then(function (result) {
        // get the workgroups in the class
        var workgroups = _this2.ConfigService.getClassmateUserInfosSortedByWorkgroupId(); // get the run id


        var runId = _this2.ConfigService.getRunId(); // the rows that will show up in the export
        ...
        if (workgroups != null) {
          ...
          for (var w = 0; w < workgroups.length; w++) {
            ...
            if (workgroup != null) {
              ...
              var componentEventCount = {}; // get the events for the workgroup
        ...
        var fileName = runId + "_events.csv"; // generate the csv file and have the client download it
    ...
    value: function createEventExportRow(columnNames, columnNameToNumber, rowCounter, workgroupId, wiseId1, wiseId2, wiseId3, studentName1, studentName2, studentName3, periodName, componentEventCount, event) {
      ...
      if (event.serverSaveTime != null) {
        ...
        if (serverSaveTime != null) {
          var serverSaveTimeString = serverSaveTime.toDateString() + " " + serverSaveTime.toLocaleTimeString(); // set the timestamp
      ...
      if (component != null) {
        // set the component type
        row[columnNameToNumber["Component Type"]] = component.type; // get the prompt with the html tags removed
      ...
      row[columnNameToNumber["Group Event Counter"]] = revisionCounter; // increment the revision counter

      componentEventCount[nodeIdAndComponentId] = revisionCounter + 1; // set the context
      ...
      row[columnNameToNumber["Event Data"]] = event; // get the pretty printed representation of the event

      var response = this.getEventResponse(event); // set the response
      ...
      if (event != null) {
        if (event.event == "branchPathTaken") {
          ...
          if (event.data != null && event.data.toNodeId != null) {
            // get the toNodeId
            var toNodeId = event.data.toNodeId; // get the step number and title of the toNodeId
    ...
    value: function exportNotebookItems(exportType) {
      ...
      this.TeacherDataService.getExport(exportType).then(function (result) {
        var notebookItems = result;
        var columnNames = ['ID', 'Teacher Username', 'Run ID', 'Period ID', 'Period Name', 'Project ID', 'Node ID', 'Component ID', 'Step Number', 'Step Title', 'Component Part Number', 'Component Type', 'Client Save Time', 'Server Save Time', 'Workgroup ID', 'WISE ID 1', 'WISE ID 2', 'WISE ID 3', 'Content', 'Note Item ID', 'Type', 'Response'];
        ...
        try {
          for (var _iterator = notebookItems[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            var notebookItem = _step.value;
            rows.push(_this3.createExportNotebookItemRow(columnNames, columnNameToNumber, notebookItem));
          }
        } catch (err) {
        ...
        if (exportType === 'latestNotebookItems') {
          fileName = "".concat(runId, "_latest_notebook_items.csv");
        } else if (exportType === 'allNotebookItems') {
          fileName = "".concat(runId, "_all_notebook_items.csv");
        }
    ...
    key: "createExportNotebookItemRow",
    value: function createExportNotebookItemRow(columnNames, columnNameToNumber, notebookItem) {
      var row = new Array(columnNames.length);
      row.fill(' ');
      ...
      var component = this.ProjectService.getComponentByNodeIdAndComponentId(notebookItem.nodeId, notebookItem.componentId);
      ...
      var position = this.ProjectService.getComponentPositionByNodeIdAndComponentId(notebookItem.nodeId, notebookItem.componentId);

      if (position != -1) {
        row[columnNameToNumber['Component Part Number']] = position + 1;
      }

      row[columnNameToNumber['Client Save Time']] = this.UtilService.convertMillisecondsToFormattedDateTime(notebookItem.clientSaveTime);
      row[columnNameToNumber['Server Save Time']] = this.UtilService.convertMillisecondsToFormattedDateTime(notebookItem.serverSaveTime);
      ...
      var userInfo = this.ConfigService.getUserInfoByWorkgroupId(notebookItem.workgroupId);
      ...
      if (notebookItem.localNotebookItemId !== 'teacherReport') {
        var student1 = userInfo.users[0];
        var student2 = userInfo.users[1];
        var student3 = userInfo.users[2];
      ...
      var responseJSON = JSON.parse(notebookItem.content);
    ...
    value: function exportNotifications() {
      ...
      this.TeacherDataService.getExport('notifications').then(function (result) {
        ...
        var columnNames = ['ID', 'Teacher Username', 'Run ID', 'Period ID', 'Period Name', 'Project ID', 'Node ID', 'Component ID', 'Step Number', 'Step Title', 'Component Part Number', 'Component Type', 'Server Save Time', 'Time Generated', 'Time Dismissed', 'From Workgroup ID', 'To Workgroup ID', 'WISE ID 1', 'WISE ID 2', 'WISE ID 3', 'Data', 'Group ID', 'Type', 'Message'];
        var columnNameToNumber = {};
        var headerRow = [];
        ...
        try {
          for (var _iterator2 = notifications[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            rows.push(_this4.createExportNotificationRow(columnNames, columnNameToNumber, notification));
          }
        } catch (err) {
        ...
        var fileName = "".concat(runId, "_notifications.csv");

        _this4.generateCSVFile(rows, fileName);

        _this4.hideDownloadingExportMessage();
      });
    }
  }, {
    key: "createExportNotificationRow",
    value: function createExportNotificationRow(columnNames, columnNameToNumber, notification) {
      var row = new Array(columnNames.length);
      row.fill(' ');
      ...
      var component = this.ProjectService.getComponentByNodeIdAndComponentId(notification.nodeId, notification.componentId);
      row[columnNameToNumber['Component Type']] = component.type;
      ...
      row[columnNameToNumber['Component Part Number']] = this.ProjectService.getComponentPositionByNodeIdAndComponentId(notification.nodeId, notification.componentId) + 1;
      row[columnNameToNumber['Server Save Time']] = this.UtilService.convertMillisecondsToFormattedDateTime(notification.serverSaveTime);
      row[columnNameToNumber['Time Generated']] = this.UtilService.convertMillisecondsToFormattedDateTime(notification.timeGenerated);

      if (notification.timeDismissed != null) {
        row[columnNameToNumber['Time Dismissed']] = this.UtilService.convertMillisecondsToFormattedDateTime(notification.timeDismissed);
      }
      ...
      var userInfo = this.ConfigService.getUserInfoByWorkgroupId(notification.toWorkgroupId);
      row[columnNameToNumber['Period Name']] = userInfo.periodName;
      row[columnNameToNumber['Teacher Username']] = this.ConfigService.getTeacherUserInfo().username;
      row[columnNameToNumber['Project ID']] = this.ConfigService.getProjectId();
      var student1 = userInfo.users[0];
    ...
    value: function getSelectedNodesToExport() {
      var selectedNodes = []; // loop through all the import project items
        ...
        if (nodeItem.node != null) {
          ...
          if (node.ids != null) {
            ...
            for (var n = 0; n < node.ids.length; n++) {
              // get a child step id
              var nodeId = node.ids[n]; // get a child step item

              var childNodeItem = this.projectIdToOrder[nodeId]; // set the checked value for the step item

              childNodeItem.checked = nodeItem.checked; // get all the components in the step
    ...
    value: function previewProject() {
      var previewProjectURL = this.ConfigService.getConfigParam('previewProjectURL'); // open the preview step in a new tab
    ...
    value: function previewNode(node) {
      if (node != null) {
        // get the node id
        var nodeId = node.id; // get the preview project url

        var previewProjectURL = this.ConfigService.getConfigParam('previewProjectURL'); // create the url to preview the step

        var previewStepURL = previewProjectURL + "#/vle/" + nodeId; // open the preview step in a new tab
    ...
    value: function exportOneWorkgroupPerRow() {
      ...
      this.TeacherDataService.getExport("oneWorkgroupPerRow", selectedNodes).then(function (result) {
        ...
        var rows = []; // get the project id

        var projectId = _this6.ConfigService.getProjectId(); // get the project title


        var projectTitle = _this6.ProjectService.getProjectTitle(); // get the run id


        var runId = _this6.ConfigService.getRunId();

        var startDate = "";
        var endDate = ""; // get the column ids that we will use for this export

        var columnIds = _this6.getColumnIdsForOneWorkgroupPerRow(selectedNodesMap); // get all the step node ids


        var nodeIds = _this6.ProjectService.getFlattenedProjectAsNodeIds(); // the headers for the description row


        var descriptionRowHeaders = ["Workgroup ID", "WISE ID 1", "Student Name 1", "WISE ID 2", "Student Name 2", "WISE ID 3", "Student Name 3", "Class Period", "Project ID", "Project Name", "Run ID", "Start Date", "End Date"]; // generate the mapping from column id to column index

        var columnIdToColumnIndex = _this6.getColumnIdToColumnIndex(columnIds, descriptionRowHeaders); // generate the top rows that contain the header cells


        var topRows = _this6.getOneWorkgroupPerRowTopRows(columnIds, columnIdToColumnIndex, descriptionRowHeaders); // add the top rows


        rows = rows.concat(topRows); // get the workgroups in the class

        var workgroups = _this6.ConfigService.getClassmateUserInfosSortedByWorkgroupId(); // loop through all the workgroups
        ...
        for (var w = 0; w < workgroups.length; w++) {
          ...
          if (workgroup != null) {
            ...
            workgroupRow.fill(" "); // get the workgroup information
            ...
            workgroupRow[columnIdToColumnIndex["End Date"]] = endDate; // loop through all the steps

            for (var n = 0; n < nodeIds.length; n++) {
              var nodeId = nodeIds[n]; // get all the components in the step
                ...
                for (var c = 0; c < components.length; c++) {
                  ...
                  if (component != null) {
                    ...
                    if (_this6.exportComponent(selectedNodesMap, nodeId, componentId)) {
                      ...
                      var columnIdPrefix = nodeId + "-" + componentId; // get the latest component state
                      ...
                      if (componentState != null) {
                        ...
                        if (_this6.includeStudentWorkTimestamps) {
                          ...
                          if (componentState.serverSaveTime != null) {
                            // get the time stamp as a pretty printed date time string
                            var formattedDateTime = _this6.UtilService.convertMillisecondsToFormattedDateTime(componentState.serverSaveTime); // set the time stamp string e.g. Wed Apr 06 2016 9:05:38 AM
                          ...
                          if (latestComponentAnnotations != null) {
                            ...
                            if (scoreAnnotation != null) {
                              if (_this6.includeScoreTimestamps) {
                                ...
                                var scoreTimestamp = _this6.UtilService.convertMillisecondsToFormattedDateTime(scoreAnnotation.serverSaveTime); // set the score timestamp
                              ...
                              if (_this6.includeScores) {
                                ...
                                if (scoreAnnotation.data != null && scoreAnnotation.data.value != null) {
                                  var scoreValue = scoreAnnotation.data.value; // set the score
                            ...
                            if (commentAnnotation != null) {
                              if (_this6.includeCommentTimestamps) {
                                ...
                                var commentTimestamp = _this6.UtilService.convertMillisecondsToFormattedDateTime(commentAnnotation.serverSaveTime); // set the comment timestamp
                              ...
                              if (_this6.includeComments) {
                                ...
                                if (commentAnnotation.data != null && commentAnnotation.data.value != null) {
                                  var commentValue = commentAnnotation.data.value; // set the comment
              ...
              if (_this6.exportNode(selectedNodesMap, nodeId)) {
                ...
                if (_this6.ProjectService.isBranchPoint(nodeId)) {
                  ...
                  if (latestBranchPathTakenEvent != null && latestBranchPathTakenEvent.data != null && latestBranchPathTakenEvent.data.toNodeId != null) {
                    ...
                    toNodeId = latestBranchPathTakenEvent.data.toNodeId; // get the title of the step
        ...
        var fileName = runId + "_one_workgroup_per_row.csv"; // generate the csv file and have the client download it
    ...
    value: function getColumnIdsForOneWorkgroupPerRow(selectedNodesMap) {
      var columnIds = []; // get all the step node ids in order
      ...
      if (nodeIds != null) {
        ...
        for (var n = 0; n < nodeIds.length; n++) {
          var nodeId = nodeIds[n]; // get the components in the step
    ...
    value: function getColumnIdToColumnIndex(columnIds, descriptionRowHeaders) {
      ...
      for (var d = 0; d < descriptionRowHeaders.length; d++) {
        ...
        var descriptionRowHeader = descriptionRowHeaders[d]; // set the column index for the description column
    ...
    value: function getOneWorkgroupPerRowTopRows(columnIds, columnIdToColumnIndex, descriptionRowHeaders) {
      ...
      var numColumns = descriptionRowHeaders.length + 1 + columnIds.length; // create the top rows
      ...
      componentIdRow.fill(" ");
      columnIdRow.fill(" ");
      descriptionRow.fill(" "); //  set the cell values for the vertical header column
      ...
      descriptionRow[descriptionRowHeaders.length] = "Description"; // fill in the headers in the description row
      ...
      if (nodeIds != null) {
        // loop through all the step node ids
        for (var n = 0; n < nodeIds.length; n++) {
          var nodeId = nodeIds[n]; // get a step title

          var stepTitle = this.ProjectService.getNodePositionAndTitleByNodeId(nodeId); // get the components in the step
          ...
          if (components != null) {
            ...
            for (var c = 0; c < components.length; c++) {
              ...
              if (component != null) {
                var componentId = component.id; // get the column prefix

                var columnIdPrefix = nodeId + "-" + componentId; // get the prompt with the html tags removed
      ...
      var topRows = []; // add all the top rows
    ...
    value: function everythingClicked() {
      ...
      this.exportStepSelectionType = 'exportAllSteps'; // settings for raw data export
    ...
    value: function exportRawData() {
      ...
      this.TeacherDataService.getExport("rawData", selectedNodes).then(function (result) {
        ...
        var data = {}; // get the workgroups in the class

        var workgroups = _this7.ConfigService.getClassmateUserInfosSortedByWorkgroupId(); // make a copy of the workgroups array to prevent referencing issues


        workgroups = _this7.UtilService.makeCopyOfJSONObject(workgroups); // loop through all the workgroups
        ...
        for (var w = 0; w < workgroups.length; w++) {
          ...
          if (workgroup != null) {
            ...
            if (_this7.includeStudentWork) {
              ...
              workgroup.studentWork = []; // get all the component states for the workgroup
            ...
            if (_this7.includeAnnotations) {
              ...
              workgroup.annotations = []; // get all the annotations for the workgroup
            ...
            if (_this7.includeEvents) {
              ...
              workgroup.events = [];
              var events = []; // get all the events for the workgroup
        ...
        data.workgroups = workgroups; // get the data as a JSON string

        var dataJSONString = angular.toJson(data, 4); // get the data JSON string as a blob

        var blob = new Blob([dataJSONString]); // generate a file and download it to the user's computer
    ...
    value: function removeNamesFromWorkgroup(workgroup) {
      ...
      try {
        for (var _iterator3 = workgroup.users[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
    ...
    value: function canExportComponentDataType(componentType) {
      ...
      try {
        for (var _iterator4 = this.availableComponentDataExports[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          var tempComponentType = _step4.value;
    ...
    value: function generateDiscussionComponentWorkRows(component, columnNames, columnNameToNumber, nodeId) {
      ...
      for (var _i = 0, _Object$keys = Object.keys(structuredPosts); _i < _Object$keys.length; _i++) {
        var threadId = _Object$keys[_i];
        ...
        if (topLevelPost.replies != null) {
          ...
          try {
            for (var _iterator6 = topLevelPost.replies[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
              var replyPost = _step6.value;
    ...
    value: function exportMatchComponent(nodeId, component) {
      ...
      this.showDownloadingExportMessage(); // request the student data from the server and then generate the export

      this.TeacherDataService.getExport("allStudentWork").then(function (result) {
        // the column names in the header row
        var columnNames = []; // mapping from column name to column number

        var columnNameToNumber = {}; // the rows that will be in the export

        var rows = []; // add the header row to the rows

        rows.push(_this9.generateMatchComponentHeaderRow(component, columnNames, columnNameToNumber)); // add the student work rows

        rows = rows.concat(_this9.generateMatchComponentWorkRows(component, columnNames, columnNameToNumber, nodeId)); // generate the file name of the csv file
    ...
    value: function populateMatchColumnNames(component, columnNames, columnNameToNumber) {
      ...
      for (var c = 0; c < defaultMatchColumnNames.length; c++) {
        // get a column name
        var defaultMatchColumnName = defaultMatchColumnNames[c]; // add a mapping from column name to column number

        columnNameToNumber[defaultMatchColumnName] = c; // add the column name to the header row
      ...
      if (this.includeCorrectnessColumns && this.MatchService.hasCorrectAnswer(component)) {
        ...
        try {
          for (var _iterator9 = component.choices[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
    ...
    value: function generateMatchComponentHeaderRow(component, columnNames, columnNameToNumber) {
      ...
      try {
        for (var _iterator10 = columnNames[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
          var columnName = _step10.value;
          // add the column name to the header row
          headerRow.push(columnName);
        }
      } catch (err) {
    ...
    value: function generateMatchComponentWorkRows(component, columnNames, columnNameToNumber, nodeId) {
      var componentId = component.id; // get the workgroups in the class

      var workgroups = this.ConfigService.getClassmateUserInfosSortedByWorkgroupId(); // the rows that will show up in the export
      ...
      try {
        for (var _iterator11 = workgroups[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
          var workgroup = _step11.value;
    ...
    value: function generateMatchComponentWorkRowsForWorkgroup(component, workgroup, columnNames, columnNameToNumber, nodeId, componentId, rowCounter) {
      var rows = []; // get the workgroup information
    ...
    value: function generateMatchComponentWorkRow(component, columnNames, columnNameToNumber, rowCounter, workgroupId, wiseId1, wiseId2, wiseId3, studentName1, studentName2, studentName3, periodName, componentRevisionCounter, matchComponentState) {
      ...
      try {
        for (var _iterator12 = matchComponentState.studentData.buckets[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
          ...
          try {
            for (var _iterator13 = bucket.items[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
...
var _default = DataExportController;
