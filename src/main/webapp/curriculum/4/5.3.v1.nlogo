...
globals [ sky-top earth-top temperature num-CO2 num-clouds starter albedo frozen? sun-brightness]

to setup
  ca
  setup-world
  set temperature 5  ;; start with a cold earth
  set albedo albedo* / 100
...
to execute 
  ...
  every 1 [
    if temperature > 0 [ set frozen? "No" set albedo albedo* / 100 ] ;; the slider controls the albedo above freezing
    if temperature < 0 [ set frozen? "YES" set albedo .95 ]  ;; if the temperature falls below zero, freeze the earth
...
to setup-world
  ...
  ask patches [                           ;; set colors of the world
   ...
   create-heat 60 [ set heading random 360 ;; start with some heat energy in the earth
     ...
     set breed heat    
     set shape "dot"  ]
   create-mountains 1 [ set shape "mountain" setxy 14 -4 set size 10 ]
   add-CO2 ;; start off with some CLO2
end      
...
to run-heat    ;; advances the heat energy turtles
  set temperature .99 * temperature + .01 * (-5 + .3 * count heat) ;; the temperature is related to the number of heat turtles
  plot temperature
  ask heat [
    fd .5 * ( random 11 ) / 10
...
to add-CO2  ;; adds 10 CO2 molecules to atmosphere at top of mountain
  let width sky-top - earth-top
...
to run-CO2
  ...
  ask CO2 [
    ...
    fd .004 * (5 + random 10) ;; move forward a bit
