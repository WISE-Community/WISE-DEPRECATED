...
globals [ sky-top earth-top temperature num-CO2 num-clouds starter albedo sun-brightness atmo first-time]
...
to execute 
  ...
  if first-time = 0 [ setup ]  ;; if the user tries to execute without reseting, do it for him/her
  ask clouds [ fd .3 * (0.1 + (3 + cloud-num) / 10) ]  ; move clouds along
  run-sunshine 
  every .5 [
    ; ask patches [update-albedo]
    if ( atmo xor atmosphere ) [  ;; only if the atmosphere button has changed update the atmosphere
      draw-atmosphere ]
    ]  
...
to update-albedo
  ; if temperature < 0 [set albedo .95] ; the earth freezes
...
to setup-world
  ...
  draw-atmosphere
  create-heat 80 [ set heading random 360 ;; start with some heat energy in the earth
     ...
     set breed heat    
     set shape "dot"  ]
   repeat 50 [ create-sunray 1 ;; create some initial sunrays, too
      [ set heading 160
      set color yellow
      setxy min-pxcor + random (max-pxcor - min-pxcor) earth-top + random (max-pycor - earth-top) ]]
     ;; start the sunrays anywhere
       set-plot-y-range 0 25
  set-plot-x-range 0 5000
end 

to draw-atmosphere  ;; creates an atmosphere if the corresponding switch is set;; otherwise leaves it blank
   set atmo atmosphere  ;; atmo remembers the state of the atmosphere
   ifelse atmosphere [  ;; do this block if there is an atmosphere
    ...
    add-CO2  ;; adds 50 water molecules to the atmosphere
    add-CO2 
    add-cloud
    add-cloud
  ]
  [      ;; execute this loop if there is no atmosphere
    ask patches [                           ;; set colors of the world
      if (pycor > earth-top) [ set pcolor black ]
      if (pycor < earth-top) [ set pcolor red + 3 ]  
      ] 
    remove-CO2
    remove-CO2
...
to run-heat    ;; advances the heat energy turtles
  set temperature .99 * temperature + .01 * (-8 + .2 * count heat) ;; the temperature is related to the number of heat turtles
  plot temperature
  ask heat [
    fd .5 * ( random 11 ) / 10
    ...
    if (ycor >= earth-top ) [  ;; if heading back into sky
      ifelse (temperature > -40 + random 200)    ;; select some to escape, more if it is hot
        [ set breed IR
...
to add-CO2  ;; randomly adds 50 water molecules to atmosphere
  let width sky-top - earth-top
...
to remove-CO2 ;; randomly remove 50 CO2 molecules
  repeat 50 [
    if (count CO2 > 0 ) [
      ask one-of CO2 [ die ]]]
  set num-CO2 count CO2
...
WHAT IS IT?
-----------
What would happen to the Earth's temperature if there was no atmosphere? This model illustrates what could happen. 

This model is based on a model of energy flow in the earth. It shows the earth as rose colored. On the earth surface is a green strip. Above that is a blue atmosphere and black space at the top. 

The atmosphere has water vapor that acts like a greenhouse gas, absorbing and re-radiating infrared light. When the atmosphere is removed, so is this greenhouse gas. Water molecules are shown in the atmosphere.  
...
HOW TO USE IT
-------------
To see what happens, slow the model down with the horizontal slider above the model. 

THINGS TO NOTICE
----------------
If the earth temperature goes below freezing, all water freezes. How does this affect the color of the surface. What is the affect on light falling on the earth? Can you see two-step cooling? What causes this? 
...
----------------------
The original model was created Nov 19, 2005 by Robert Tinker for the TELS project. Updated Jan 9, 2006. Updated and simplified to show only the impact of the atmosphere January 2010. 
