var AnnotatorAutoScore = function(){
	this.regions = [];
	this.labels = [];
	this.mappings = [];
	...
	this.initializeData = function() {
		...
		this.mappings.push(mapping1);
		this.mappings.push(mapping2);
		this.mappings.push(mapping3);
		...
		this.scoringCriteria.push(score1);
		this.scoringCriteria.push(score2);
		this.scoringCriteria.push(score3);
		...
		this.studentLabels.push(studentLabel1);
		this.studentLabels.push(studentLabel2);
	};

	/**
	 * Run the automated scoring and obtain the score and feedback
	 */
	this.runScoring = function() {
		//add the regions to the svg display
		this.addRegionsToDisplay(regions);
		
		//add the student labels to the svg display
		this.addStudentLabelsToDisplay(studentLabels);
	 ...
	 */
	this.addRegionsToDisplay = function(regions) {
		if(regions != null) {
				...
				if(region != null) {
					...
					if(shape != null) {
						...
						if(shapeType == null) {
							...
							var svgRectangle = this.createSVGRectangle(rx, ry, rwidth, rheight, 'none', 1, regionColor);
							
							//add the rectangle to the svg display
							$('#svgDisplay').append(svgRectangle);
							
							//create an svg text element
							var svgText = this.createSVGText(rx, ry + 10, regionColor, regionName);
							...
							var svgCircle = this.createSVGCircle(cx, cy, cradius, 'none', 1, regionColor);
							
							//add the circle to the svg display
							$('#svgDisplay').append(svgCircle);
							
							//create an svg text element
							var svgText = this.createSVGText(cx - cradius, cy - cradius + 10, regionColor, regionName);
	 ...
	 */
	this.createSVGRectangle = function(rx, ry, rwidth, rheight, fill, strokeWidth, stroke) {
		//create an svg rectangle element
	 ...
	 */
	this.createSVGCircle = function(cx, cy, cradius, fill, strokeWidth, stroke) {
		//create an svg circle element
	 ...
	 */
	this.createSVGText = function(x, y, fill, text) {
		//create an svg text element
	 ...
	 */
	this.addStudentLabelsToDisplay = function(studentLabels) {
	 ...
	 */
	this.matchStudentLabelsToAuthoredLabels = function(studentLabels, authoredLabels) {
	 ...
	 */
	this.doesStudentLabelMatchAuthoredLabel = function(studentLabel, authoredLabel) {
		var result = false;
	 ...
	 */
	this.isPointInRegion = function(x, y, region) {
		var result = false;
	 ...
	 */
	this.isPointInRectangle = function(x, y, rx, ry, rwidth, rheight) {
		var result = false;
	 ...
	 */
	this.isPointInCircle = function(x, y, cx, cy, cradius) {
		var result = false;
	 ...
	 */
	this.matchStudentLabelsToRegions = function(studentLabels, regions) {
	 ...
	 * ]
	 */
	this.generateLabelToRegionMappings = function(studentLabels) {
		var studentMappings = [];
	 ...
	 */
	this.calculateSatisfiedMappings = function(mappings, studentMappings) {
		var mappingResults = [];
	 ...
	 */
	this.isMappingInStudentMappings = function(mapping, studentMappings) {
		var result = false;
	 ...
	 */
	this.calculateScore = function(scoringCriteria, mappingResults) {
		var score = 0;
		var feedback = '';
	 ...
	 */
	this.checkScoringCriteria = function(scoringCriteriaObject, mappingResults) {
		var results = {
			score:null,
			feedback:null
		}
	 ...
	 */
	this.replaceMappingIdsWithValues = function(logic, mappingResults) {
		var result = logic;
	 ...
	 */
	this.sortNumericallyDescending = function(a, b) {
		return (b - a);
	};
	 ...
	 */
	this.isMappingSatisfied = function(mappingResults, mappingId) {
		var result = false;
	 ...
	 */
	this.getMaxScore = function(scoringCriteria) {
		var maxScore = 0;
...
if(typeof eventManager != 'undefined'){
    eventManager.fire('scriptLoaded', 'vle/node/draw/svg-edit/autograde/annotatorAutoScore.js');
};