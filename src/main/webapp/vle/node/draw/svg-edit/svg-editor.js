			...
			*/
			...
			(function() {
				...
				$.each(toolnames, function(i,item) {
					...
					all_tools += '#tool_' + item + (i==toolnames.length-1 ? '' : ', '); // WISE4: Edit to work with jQuery 1.9+ (https://groups.google.com/forum/?fromgroups=#!topic/svg-edit/XP67MWW4_zE)
				});
...
'					<defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
			...
			var changeSidePanelWidth = function(delta) {
				...
				$('#fit_to_canvas').mouseup(); // wise4: resize canvas to fit workarea - TODO: remove of use workareaResized event
			};
		...
		//			{sel: '#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
			...
			$('.contextMenu li').on('click.contextMenu', function(ev) { // wise4 fix for context menu clicks that were attempting to follow href
				ev.preventDefault();
			});
