...
MC.prototype.checkAnswer = function() {
	...
	this.processTeacherNotifications();
};

/**
 * Process the teacher notifications
 */
MC.prototype.processTeacherNotifications = function() {
    ...
    if (teacherNotifications != null) {
        ...
        var latestNodeVisit = this.view.getState().getCurrentNodeVisit();

        // get all the node visits for this step
        var nodeVisits = this.view.model.getState().getNodeVisitsByNodeId(nodeId);
        ...
        for (var t = 0; t < teacherNotifications.length; t++) {
            ...
            if (teacherNotification != null) {
                ...
                if (teacherNotificationType === 'attemptIncorrect') {
                    /*
                     * the attempCorrect notification activates when the student
                     * answers correctly or incorrectly on a specific attempt
                     */
                    var numberOfAttempts = 0;

                    // the attempt number we want to look for
                    ...
                    if (nodeVisits != null) {
                        ...
                        for (var nv = 0; nv < nodeVisits.length; nv++) {
                            ...
                            if (nodeStates != null) {
                                // count up the number of attempts
                                numberOfAttempts += nodeStates.length;
                            }

                            // remember the latest node state
