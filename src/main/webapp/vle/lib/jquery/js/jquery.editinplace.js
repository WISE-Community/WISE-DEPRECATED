...
$.extend(InlineEditor.prototype, {
	...
	setClosedEditorContent: function(aValue) {
		/*
		 * added by wise so that if the content is empty it will replace it with the default text
		 * this handles the case when the element is new and has never been edited and
		 * the user clicks on it but does not change the text from the empty '' value
		 */
		if($.trim(aValue) == '') {
			aValue = this.settings.default_text;
			this.didInsertDefaultText = true;
		}
	...
	handleSubmitToCallback: function(enteredText) {
		...
		if (this.settings.callback_skip_dom_reset)
			...
			/*
			 * added by wise so that when there was previous content that was not empty
			 * and the user deletes all the characters this will replace the empty
			 * string with the default text
			 */
			if($.trim(newHTML) == '') {
				newHTML = this.settings.default_text;
				this.didInsertDefaultText = true;
			}
...
if(typeof eventManager != 'undefined'){
	eventManager.fire('scriptLoaded', 'vle/lib/jquery/js/jquery.editinplace.js');
}